...
This code is to develop a recommender system that will recommend new musical artists to a user based on
their listening history. Suggesting different songs or musical artists to a user is important to many
music streaming services, such as Pandora and Spotify. In addition, this type of recommender system could
also be used as a means of suggesting TV shows or movies to a user (e.g., Netflix).
To create this system Spark (using pyspark) and a collaborative filtering technique (Alternative Least
Squares) were deployed.
'''
...
#Loading data
'''
The data considered was publicly available song data at http://www-etud.iro.umontreal.ca/~bergstrj/
audioscrobbler_data.html. However, the original data files were modified so that the code will run in a
reasonable time on a single machine. Those reduced files were uploaded into dropbox and used here.
'''
